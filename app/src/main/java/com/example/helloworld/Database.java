...
public class Database extends SQLiteOpenHelper {
    ...
    public void addTask(timetableTask t, String date) {
        checkTable(date);
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public ArrayList<timetableTask> getAllTasks(String date) {
        checkTable(date);
        ...
        if (cursor.moveToFirst()) {
            do {
                timetableTask t = new timetableTask();
    ...
    public int getNextID(String date) {
        ArrayList<timetableTask> tasks = getAllTasks(date);
        int id = 0;
        int size = tasks.size();
        if (size != 0) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public void updateTask(timetableTask t, String date) {
